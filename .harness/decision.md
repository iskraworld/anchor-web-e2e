...
## 2026-05-25: Savings Plan 권장값 ($0.373/hr) 그대로 클라이언트 제시 — Custom 보수 옵션 채택 안 함

- **선택**: Compute SP / 1년 / No upfront / **$0.373/시간** 그대로 클라이언트에게 제시 (옵션 B). 최종 진행 여부는 클라이언트 결정
- **대안 검토**:
  - A) Custom $0.30/hr 보수 — -$80~90/월, over-commit 위험 거의 0 (Claude 초기 추천)
  - **B) 권장 $0.373/hr 그대로** (선택) — -$107/월 (최대 절감)
  - C) lookback 32/93일 재분석 — 권장값 오히려 ↑ (다운사이즈 이전 데이터 포함), 추가 가치 낮음
- **선택 이유** (Eugene 결정):
  - 13일 lookback 차트에서 **권장값 $0.373/hr = baseline floor** 확인 — "2주간 시간당 이것보다 적게 쓴 적 없었음"
  - 즉 권장값 자체가 over-commit 안전선이라 추가 보수(옵션 A)는 over-conservative
  - 옵션 A 도입 시 잠재 절감 $20/월 ($240/년) 손실
  - SP 가 baseline 만 커버하므로 향후 사용량 증가 시 초과분은 자동 온디맨드 — under-commit 위험 없음
  - 사용량 감소(서비스 축소) 가능성은 베타→정식 진행 방향 고려 시 낮음
- **영향 범위**: 클라이언트 confirm 시 anchor AWS 계정에 1년 SP 1건 추가, 매시간 $0.373 청구 (월 $272 / 연 $3,268). 절감 -$107/월
- **되돌리는 방법**: SP 는 **중도 환불 불가 / 양도 불가**. 1년간 유지 필수. 사용량 감소해도 약정 금액 지속 청구
