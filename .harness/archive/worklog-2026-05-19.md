...
## Session 2026-05-19 20:29 — Neo4j 다운사이즈 전체 체크리스트 적용 후 r6i.large 유지 결정

### 작업 요약

#### 1. Neo4j 다운사이즈 사전 체크리스트 §1~§6 전체 적용 (5/12 작업 후 정확히 1주)
- 초기 추천 시 §1 디스크 I/O / 네트워크 / 30일 lookback / JVM heap / Neo4j store 단독 사이즈 누락 → 사용자 지적 후 전부 재수집
- **30일 메트릭 수집**: AWS_PROFILE=anchor, lookback 2026-04-19 ~ 2026-05-19
  - CPU: P95 3.7%, P99 8.2%, **max 51.2% 1회 스폿** (정기 배치 작업 추정)
  - 메모리 (CWAgent): P95 17.5%, P99 17.6%, swap 0
  - EBS Read: peak 39 ops/s (t3.large 174 IOPS baseline 의 22%)
  - EBS Write: peak 7 ops/s (4%)
  - Network In: peak 73.75 Mbps (t3.large 768 Mbps baseline 의 10%)
  - Network Out: peak 1.67 Mbps (0.2%)
- **컨테이너 진단 (SSM run-command)**:
  - `anchor-neo4j` (neo4j:5.13) 컨테이너 메모리 사용 2.37 GiB / 호스트 15.34 GiB
  - JVM heap_max 2GB + page cache 1GB = **합 3GB**
  - OOM 이력 0건 (journalctl + dmesg)
  - 그래프 데이터 `/data/databases/neo4j` = **7.5 GiB**
  - AMI arch: x86_64
- **체크리스트 5조건 결과**: 4/5 통과, 1조건 (page cache vs DB 사이즈) 미달이지만 다운사이즈 결정과 무관

#### 2. 다운사이즈 안 하기로 결정
- **데이터상 t3.large(8GB) 다운사이즈 가능** (-$29/월 절감 가능)
- **그러나 사용자 결정: r6i.large 유지** — 이유:
  1. 절감액($29) 대비 작업 리스크 비대칭
  2. CPU max 51.2% 1회 spot 은 정기 배치 작업 추정 → 다운사이즈 후 burst credit 소진 / OOM 위험
  3. Neo4j 권장은 "전체 DB 가 page cache 에 들어가야 이상적" — 현재 1GB / 7.5GB 이미 미달, 메모리 다운사이즈는 반대 방향
  4. 베타 단계 잔여 작업 대비 미미한 절감
- **별건 후보 등재**: Neo4j page cache 1GB → 4~8GB 튜닝 검토 (성능 향상), CPU 51.2% spike 정체 확인

#### 3. eugene-followups 갱신
- "Neo4j 메모리 사용 분석 + 사이징" 항목 `[x]` 드롭 + 30일 데이터 보존
- "5/18 Neo4j 다운사이즈에 적용" 섹션 ~~취소선~~ 처리 + 결정 명시
- "누적 절감 진행" 섹션 갱신: Neo4j 잠재 절감 제거, 남은 큰 잠재는 Savings Plan (-$150~200/월) 만
- "권한 회수" 타이밍 변경: 5/20 → **2026-05-25 경** (사용자 결정 — Savings Plan 작업과 일괄 회수)
  - 기술적으로는 SP 작업이 Billing Console 이라 IAM 무관하지만, 회수 타이밍을 묶어두는 운영적 결정
  - Elevated 권한은 이미 5/12 트림으로 Neo4j ARN 제한 + S3/DDB + SSM 만 남음

### 실패한 시도
- 1차 추천에서 디스크 I/O / 네트워크 / JVM 설정 / Neo4j store 단독 사이즈 등 핵심 항목 누락 → 체크리스트 자기 점검 부족. 사용자 지적 후 보강
- 1차 SSM 스크립트: docker stats 출력 형식 `{{.MemLimit}}` 필드 에러 + 변수 `NEO4J_CT=$(...)` 가 awsrunShellScript 의 라인 분리 실행에서 안 보이는 문제 → 단순화 + 컨테이너명 하드코딩 (anchor-neo4j) 으로 해결
- jcmd 1 GC.heap_info: Java attach socket 에러로 실패. JVM heap 정보는 컨테이너 env (NEO4J_server_memory_*) 로 대체 확보

### 다음 액션
1. (5/25 경) Savings Plan 재분석 → 권장액 $0.60~0.70/hr 수준이면 약정 검토
2. (5/25 경, SP 작업 후) IAM 인라인 정책 `anchor-rightsize-2026-05-11` + GitLab Maintainer 일괄 revoke
3. (별건 후순위) Neo4j page cache 1GB → 적정값 튜닝 검토 — 운영팀 협의
4. (정식 오픈 시) GitLab Issue #1 복구 체크리스트 9개 항목
