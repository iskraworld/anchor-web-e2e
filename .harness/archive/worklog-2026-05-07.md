...
## Session 2026-05-07 19:28 — 작업 가이드 v2 (사람/Claude 분담) + 개발팀 공지

### 작업 요약
- **work-guide-2026-05-09-v2.md** 작성 — 작업 가이드 v1 → v2 보강:
  - 작업 분담 명시: 🤖 Claude / 🤝 협업 / 👤 사람
  - Claude 작업 비율: 0% → ~70% (위험 항목 제외 모두 자동화)
  - 인라인 정책 부여 → 작업 → 회수 패턴 (CWAgent 설치 패턴 재사용)
  - 리소스 ID로 정확히 권한 제한 (잘못된 인스턴스 못 건드림)
  - "X 롤백해줘" 한마디로 Claude 자동 롤백
  - Phase별 별도 권한 (토요일 / Neo4j / Savings Plan)
  - 사람만 하는 것: ALB 삭제 (비가역) / gw01 (SPOF) / Smoke test / Savings Plan / IAM
  - Claude 자동: EBS 스냅샷, dev/was 다운사이징, ElastiCache, RDS, ASG 사전 정의, 메모리/비용 분석, 모든 롤백
- **dev-team-notice-2026-05-09.md** 작성 — 개발팀 공지 문서:
  - 변경 내역 (Phase 1-3) + 절감액 + 다운타임 표
  - 측정 근거 6가지 (CPU 0.5%, credits, RDS connection 6.4, IOPS 0.1/s, 메모리 1%, 응답시간 174ms)
  - 개발팀 영향: 코드 변경 X / 응답시간 174→200~300ms / 다운타임별 영향 / RDS 재시도 / 캐시 재구축
  - 사용자 증가 시 단계별 비용 + 작업 시간 (5k/10k/20k/50k/100k)
  - 모니터링 알람 6가지 (CPU credit / response time / connections / 5xx 등)
  - 사전 준비 3가지 (retry 로직 / batch 일정 / 알림 채널)
  - 작업 후 협조 (Smoke test / 일요일 모니터링)
  - FAQ 5개 (사용자 폭증 / 롤백 / 왜 지금 / Neo4j 1주 / SP 2주)

### 다음 액션
- (사용자) 토요일 작업 전 IAM 인라인 정책 부여 (work-guide v2 §Phase 1 사전 권한)
- (사용자) 개발팀에 dev-team-notice-2026-05-09.md 전달
- (사용자) QA 팀에 v4 prompt + TF 재작성 요청
- (Claude 대기) 토요일 권한 부여 알림 받으면 → 일괄 작업 진행
- (Claude 대기) QA 팀 결과물 받으면 → Tier 1 비교 분석
