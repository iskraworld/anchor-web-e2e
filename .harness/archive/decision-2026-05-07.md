...
## 2026-05-07 (저녁): anchor-web-e2e는 framework consumer로 전환하지 않고 source 영구 유지

- **선택**: anchor v1은 framework source로 영구 유지. consumer 전환은 anchor v2 / 신서비스에서만
- **대안 검토**:
  - A) anchor를 framework consumer로 전환: 일관성 ↑. 단 풀테스트 797 PASS 회귀 위험 + 헤더 추가로 git diff 노이즈
  - B) anchor source 유지 + 신서비스에서 consumer 검증 (선택): anchor 안전 + 진짜 third-party validation
  - C) anchor에 framework 별도 폴더로 카피 (e2e-framework/): 두 source 관리 복잡 — 비추
- **선택 이유**:
  1. anchor v1은 4 검증 사이클 완료된 검증된 source. 갈음할 가치 X
  2. framework consumer 검증은 어차피 anchor v2 / 신서비스에서 진행 — 진짜 third-party
  3. Tier 1 분석은 anchor 자체 scripts로 충분 (framework 카피본 불필요)
  4. anchor 풀테스트 베이스라인 안전 보존
- **영향 범위**: anchor-web-e2e source 그대로 + framework consumer 트랙 별도 진행
- **되돌리는 방법**: 향후 필요 시 `/e2e-framework-init` 으로 anchor도 consumer 전환 가능 (단 풀테스트 회귀 검증 필수)
