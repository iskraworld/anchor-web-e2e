...
- **대안 검토**:
  - A) 11모듈 일괄 재생성 (사용자 원안): 2-3주, 일괄 보강 함정 재현 위험 (이미 학습)
  - B) 3 Tier 점진 (선택): Tier 1 (1모듈 QA만) → Tier 2 (1모듈 spec) → Tier 3 (전체). 각 Tier 검증 게이트
  - C) anchor 검증 skip + 신서비스 직행: v4 미검증 상태로 anchor v2/신서비스 진입 → import unverified framework
- **선택 이유**:
  1. 변수 격리 (입력 동일 + prompt만 변경) 가치는 인정 — v4의 진짜 white-box 검증
  2. 11모듈 일괄은 검증 사이클 1-4에서 학습한 "일괄 보강 함정" 재현. 모듈 단위 게이트가 회귀 격리에 안전
  3. TF 모듈 추천 — 13건 AMBIGUOUS + 검색/필터/모달 패턴 다양 → v4 룰 [10]~[14] 모두 작동 검증
  4. 사전 자료 접근성 미확인 — 답변 받고 진행
- **영향 범위**: Tier 1만 시작 시 docs/qa/QA_TF_*.md 신규 버전 + 비교 리포트. Tier 2/3는 게이트 통과 후 결정
- **되돌리는 방법**: Tier 1 결과가 v4 효과 없으면 prompt v5 보강 후 재시작. 자료 접근 불가 시 신서비스 직행 (옵션 C 회귀)

---

## 2026-05-06: 154건 AMBIGUOUS_DOC 처리 방식 결정

- **선택**: QA 생성 프롬프트 보강(v4)을 통한 근본 해결
- **대안 검토**: 
  - ① 154건 일괄 수동 리뷰: 2-3시간 소요, 사장될 가능성 높음
  - ② 프롬프트 자체 업데이트: 향후 생성 건들에 자동 적용, 일회성 노력으로 지속 효과
- **선택 이유**: 패턴 분석(모호 동사 87건, 빈 셀 12건 등)으로 근본 원인이 명확했기에, 사람의 일회성 판단보다 자동화된 정량 체크(규칙 [10]~[14])가 장기적으로 효율적
- **영향 범위**: `docs/anchor-e2e-v2/qa-doc-generation-prompt.md` 파일 + 향후 anchor v2 docs 생성 시점부터 모든 신규 QA docs
- **되돌리는 방법**: v3 프롬프트로 원복하거나, 규칙 [10]~[14]를 선택적으로 제거/수정


## 2026-05-06: Cycle 4 N 1건 처리 — 패치 + 종료 (옵션 A)

- **선택**: TF-1-07 단일 패치 후 검증 사이클 종료 (5차 사이클 요청 안 함)
- **대안 검토**:
  - A) 패치 + 종료 (선택): TF-1-07 fix 후 framework v3.1 정착 선언, 검증자 추가 부담 0
  - B) 패치 + 5차 사이클: 검증자 +7분 부담, N=0 확인 가치 vs 한계효용 추가 감소 (6.7% → ~3% 예상)
  - C) 패치 없이 종료: TF-1-07 잔존 결함 인정, 즉시 종료
- **선택 이유**:
  1. N 1건이 새 패턴 아님 — 기존 §7 도메인 정답 양방향의 적용 누락 케이스. framework 보강 불필요
  2. 한계효용 6.7% → ~3%는 검증자 부담 대비 ROI 낮음 (수렴 곡선 충분히 검증됨)
  3. 패치 후 풀테스트 0 회귀 → 코드 자산도 깔끔하게 마무리
- **영향 범위**: tests/qa/tf/tf.spec.ts (TF-1-07 5단계 흐름), state.md 진행 상황 (검증 사이클 완료)
- **되돌리는 방법**: 신서비스 적용 중 같은 패턴 또 누락 발견 시 framework 재검토 / 추가 사이클은 새 프로젝트에서 재시작

---

## 2026-05-05 (오후): 검증 사이클 종료 결정 재검토 — Cycle 4 진행

- **선택**: 같은 날 오전의 "사이클 종료" 결정을 뒤집고 Cycle 4 진행. 단, 검증자 거절 옵션 명시
- **대안 검토**:
  - A) 종료 결정 유지 (오전 결정 존중): framework 80%로 신서비스 진입 → 검증되지 않은 framework import 위험
  - B) Cycle 4 진행 (선택): 검증자 7분 추가 부담, N=0 또는 long tail 데이터 확보
- **선택 이유**:
  1. "framework 정착"은 N=0 데이터 없이 주장 불가 (20% 잔존 = 5개 중 1개 틀림)
  2. "anchor는 학습 환경" 재정의는 사후 합리화 — state.md 원래 목표는 anchor 회귀 디버깅 포함
  3. "한계효용 급감" 예측은 데이터 아니라 추측. 4차 결과 자체가 종료 vs 계속 결정 근거
  4. 검증자 부담은 7분 1회, 동의 거절 시 자연 종료 가능
- **영향 범위**: docs/verify-samples-2026-05-05-c4.md, scripts/sample-verify.mjs (--exclude-from), state.md 진행 상황
- **되돌리는 방법**: 검증자 거절 → 자동으로 옵션 A로 전환, 오전 종료 결정 유효

---

## 2026-05-05 (오전): 검증 사이클 종료 — anchor에서 멈추고 신서비스로 진행

- **선택**: 검증자 N 3건 fix까지 완료 후 추가 검증 사이클 중단. 신서비스(사주톡 등) 적용으로 진행
- **대안 검토**:
  - A) 한 사이클 더 (확실한 N=0 마무리): 검증자 7분 추가 부담, 1-2개 패턴 추가 발견 가능성
  - B) 지금 멈추기 (선택): 사용자 검증자 부담 인지, 신서비스 적용으로 진짜 검증
  - C) 자동화만 강화 (사람 검증 X): navigation shortcut 47건 등 점진 정리, 의미 위험은 못 잡음
- **선택 이유**:
  1. 한계효용 급감 (60% → 20% → ~10% 예상)
  2. 사람 시간이 가장 비싼 자원 (3 사이클 누적 검증자 시간 ~45분 + 컨텍스트 스위치)
  3. 현재 자산이 신서비스 적용에 충분 — anchor에서 더 다듬는 것보다 새 서비스에서 진짜 검증
  4. "충분 수준" 인식이 framework 설계의 진짜 완성. 무한 사이클은 자체 모순
  5. 잠재 패턴은 신서비스에서 자연 발견 — anchor 한정 도메인 패턴은 사주톡에 큰 가치 없음
- **영향 범위**: anchor 한정 추가 검증 X. framework v3 자산(automation-patterns §11/§12/§13 + qa-doc-generation-prompt + sample-verify.mjs)이 신서비스 적용 입력
- **되돌리는 방법**: 신서비스 적용 중 같은 패턴 다수 발견 시 anchor에서 추가 검증 사이클 재개 가능 (가능성 낮음)
